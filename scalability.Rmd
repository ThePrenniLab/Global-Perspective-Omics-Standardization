...
```{r, include=TRUE}
#Read in data frame
df <- read.csv("scalability_to-join.csv")
df_original <- df
columns_to_bin <- 8:15
...
# Apply row-wise
binned_selected <- t(apply(df_original[, columns_to_bin], 1, bin_individual_safe))

# Add scaling to data frame
df_binned <- df_original
...
results <- df_long %>%
  ...
  summarise(
    chisq_pval = tryCatch(
      chisq.test(table(global_group, BinnedScore))$p.value,
      error = function(e) NA
    )
  )

write.csv(results, "scalability_significance.csv")
